...
def generate_chebyshev_features(x, chebyshev_freq_min, chebyshev_freq_max):
    chebyshev_features = []
    theta = torch.pi * x[:, 0]
    dx = torch.sin(theta).unsqueeze(1)
    left_end = torch.abs(theta) < 1e-8
    right_end = torch.abs(theta - torch.pi) < 1e-8
    chebyshev_arg = torch.cos(torch.pi * x)
    uk_minus_2 = torch.sin((chebyshev_freq_min + 1) * torch.pi * x) / dx 
    uk_minus_2[left_end] = chebyshev_freq_min + 1
    uk_minus_2[right_end] = (chebyshev_freq_min + 1) * (-1)**chebyshev_freq_min
    uk_minus_1 = torch.sin((chebyshev_freq_min + 2) * torch.pi * x) / dx 
    uk_minus_1[left_end] = chebyshev_freq_min + 2
    uk_minus_1[right_end] = (chebyshev_freq_min + 2) * (-1)**(chebyshev_freq_min + 1)
    for k in range(chebyshev_freq_min, chebyshev_freq_max + 1):
        if k == chebyshev_freq_min:
            current_uk = uk_minus_2
        elif k == chebyshev_freq_min + 1:
            current_uk = uk_minus_1
        else:
            current_uk = 2 * chebyshev_arg * uk_minus_1 - uk_minus_2
            uk_minus_2 = uk_minus_1
            uk_minus_1 = current_uk
        chebyshev_features.append(current_uk)
        qq = torch.cat(chebyshev_features, dim=1)
    return torch.cat(chebyshev_features, dim=1)

def generate_chebyshev_features2(x, chebyshev_freq_min, chebyshev_freq_max):
    theta = torch.pi * x[:, 0]
