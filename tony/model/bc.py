...
def get_g0_func(
    ...
    interpolation_type: str = "multilinear"
    ...
    if interpolation_type == "hermite_cubic_2nd_deriv":
    ...
    elif interpolation_type == "multilinear":
        boundary_values = {}
        ...
        raise ValueError(f"Unknown interpolation_type: {interpolation_type}. Choose 'multilinear' or 'hermite_cubic_2nd_deriv'.")


def _psi_tensor(t: torch.Tensor) -> torch.Tensor:
    """
    Helper function for the C-infinity bump function construction: exp(-1/t) for t > 0, else 0.
    """
...
def get_d_func(domain_dim: int, domain_bounds: Union[Tuple[float, float], Tuple[Tuple[float, float], ...]], 
    ...
    if d_type == "quadratic_bubble":
        ...
        if domain_length is None:
             # This should not happen if domain_dim is 1, but as a safeguard.
        ...
        def d_sin_half_period_val(x: torch.Tensor) -> torch.Tensor:
            x_val = x[:, 0]
            # Translate x so min_bounds[0] becomes 0, then scale by pi/L
